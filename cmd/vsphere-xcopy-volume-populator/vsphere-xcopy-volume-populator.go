...
func main() {
	...
	hll := HostLeaseLocker{clientset: clientSet}
...
type HostLeaseLocker struct {
	...
	// leaseDuration is how long the lease is held (in seconds). Default: 60 seconds
	...
	// renewInterval is how often to renew the lease while work is running. Default: 2.5 seconds (1/4 of lease duration)
	renewInterval time.Duration
	// maxConcurrentHolders is the maximum number of concurrent lease holders per host. Default: 2
	// Can be configured via HOST_LEASE_MAX_CONCURRENT env var
...
// initializeConfig initializes the HostLeaseLocker configuration from environment variables
func (h *HostLeaseLocker) initializeConfig() {
	if h.namespace == "" {
		if leaseNs := os.Getenv("HOST_LEASE_NAMESPACE"); leaseNs != "" {
			h.namespace = leaseNs
		} else {
			h.namespace = "openshift-mtv"
		}
	}

	if h.leaseDuration == 0 {
		h.leaseDuration = 10 * time.Second
	...
	if h.retryInterval == 0 {
		h.retryInterval = 10 * time.Second
	}

	if h.renewInterval == 0 {
		// Renew at 1/4 of the lease duration to ensure we don't lose the lease
		h.renewInterval = h.leaseDuration / 4
	}

	if h.maxConcurrentHolders == 0 {
		h.maxConcurrentHolders = 2
		if maxStr := os.Getenv("HOST_LEASE_MAX_CONCURRENT"); maxStr != "" {
			if max, err := strconv.Atoi(maxStr); err == nil && max > 0 {
				h.maxConcurrentHolders = max
			}
		}
	}
}
